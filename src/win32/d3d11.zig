...
pub fn paint(
    ...
    const viewport_cell_count: XY(u32) = .{
        .x = @divTrunc(client_size.x + font.cell_size.x - 1, font.cell_size.x),
        .y = @divTrunc(client_size.y + font.cell_size.y - 1, font.cell_size.y),
    };

    {
        ...
        config.viewport_cell_count[0] = viewport_cell_count.x;
        config.viewport_cell_count[1] = viewport_cell_count.y;
    }

    const cell_count = viewport_cell_count.x * viewport_cell_count.y;
    _ = cell_count;
    ...
    // we first loop through all the cells to ensure they are already rendered before
    // we map in the cell array and write into it so as not to keep it mapped for longer
    // than necessary
    // TODO: maybe add a performance timer for this loop as DirectWrite can be sloooooow
    for (0..@min(screen.height, viewport_cell_count.y)) |row| {
        const row_offset = row * screen.width;
        for (0..@min(screen.width, viewport_cell_count.x)) |col| {
    ...
    if (state.shader_cells.count > 0) {
        ...
        for (0..@min(screen.height, viewport_cell_count.y)) |row| {
            ...
            const dst_row_offset = row * viewport_cell_count.x;
            for (0..@min(screen.width, viewport_cell_count.x)) |col| {
...
fn shaderColorFromVaxis(color: vaxis.Color) u32 {
    return switch (color) {
        .default => 0,
        .index => |idx| return @as(u32, xterm.colors[idx]) << 8 | 0xff,
        .rgb => |rgb| (@as(u32, rgb[0]) << 24) |
            (@as(u32, rgb[1]) << 16) |
            (@as(u32, rgb[2]) << 8) | 0xff,
    };
}
