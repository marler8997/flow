...
const State = struct {
    ...
    // todo: propagate this color to the render backend
    //       for d3d11 we want to call SetBackgroundColor on the swap chain
    background: ?u32 = null,
...
fn WndProc(
    ...
    switch (msg) {
        ...
        //     // the dpi change is effective, so, we get the current cell size from the font
        //     // and re-scale it based on the dpi ourselves
        //     const font_receipt = render.updateFont(win32.dpiFromHwnd(hwnd), getFontSize());
        //     const current_cell_size = render.getCellSize(font_receipt);
