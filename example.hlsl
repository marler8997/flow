...
float4 PixelMain(float4 sv_pos : SV_POSITION) : SV_TARGET {
    ...
    uint index = grid_pos.y * viewport_cell_dim.x + grid_pos.x;
    ...
    if (DEBUG_MODE == DEBUG_MODE_CHECKERBOARD) {
        uint cell_count = viewport_cell_dim.x * viewport_cell_dim.y;
    ...
    // Calculate the position of this glyph in the texture
    uint glyph_index = cell.glyph_index;
    uint2 glyph_cell_pos = uint2(
        glyph_index % cells_per_row,
        glyph_index / cells_per_row
    );

    // Calculate final texture coordinates
    uint2 texture_coord = glyph_cell_pos * cell_size + cell_pixel;
    //uint2 cell_count = texture_size / cell_size;
    //uint2 texture_coord = uint2(index % cell_count.x, index / cell_count.x);
