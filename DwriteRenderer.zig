...
text_format: ?*win32.IDWriteTextFormat = null,
// transfer_view: *win32.ID3D11ShaderResourceView,
...
pub fn setFont(
    ...
    if (self.text_format) |text_format| {
        _ = text_format.IUnknown.Release();
        self.text_format = null;
    }
    {
        var text_format: *win32.IDWriteTextFormat = undefined;
        const hr = self.dwrite_factory.CreateTextFormat(
            name,
            null,
            .NORMAL, //weight
            .NORMAL, // style
        ...
        if (hr < 0) std.debug.panic(
            ...
            .{ std.unicode.fmtUtf16le(name), height, @as(u32, @bitCast(hr)) },
        );
        self.text_format = text_format;
    }
    errdefer _ = self.text_format.IUnknown.Release();
...

    {
        ...
        {
            const hr = self.dwrite_factory.CreateTextLayout(
                win32.L("█"),
                1,
                self.text_format,
        ...
        return .{
            .x = @as(u16, @intFromFloat(@ceil(metrics.width))),
            .y = @as(u16, @intFromFloat(@ceil(metrics.height))),
        };
    }
}
    ...
    pub fn update(
        ...
        if (self.cached) |cached| {
            ...
            std.log.info(
                ...
                .{ cached.size.x, cached.size.x, size.x, size.y },
            );
        ...
        // TODO: does this need to be kept alive?
...
pub fn render(
    ...
    const text_format = self.text_format orelse @panic("render called without first calling setFont");
