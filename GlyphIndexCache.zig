...
pub fn init(allocator: std.mem.Allocator, capacity: u32) error{OutOfMemory}!GlyphCache {
    var result: GlyphCache = .{
        .map = .{},
...
pub fn clearRetainingCapacity(self: *GlyphCache) void {
...
pub fn deinit(self: *GlyphCache, allocator: std.mem.Allocator) void {
    allocator.free(self.nodes);
    self.map.deinit(allocator);
}

pub fn isFull(self: *const GlyphCache) bool {
...
pub fn reserve(self: *GlyphCache, allocator: std.mem.Allocator, codepoint: u21) error{OutOfMemory}!union(enum) {
...
fn moveToBack(self: *GlyphCache, index: u32) void {
...
fn testValidate(self: *const GlyphCache, seen: []bool) !void {
    for (seen) |*s| {
        s.* = false;
    }
...
test "GlyphCache basic functionality" {
    const testing = std.testing;
    const allocator = testing.allocator;

    var validation_buf: [3]bool = undefined;
    var cache = try GlyphCache.init(allocator, 3);
